// ─────────────────────────────────────────
//  MATRIZ CONFIG — Standalone (no depende de otro .gs)
//  Si tienes IPERC_Gemini_v2.gs en el mismo proyecto,
//  estas funciones se fusionan automaticamente.
// ─────────────────────────────────────────
...
function leerMatrizConfig() {
  ...
  try {
    ...
    /* Return format compatible with HTML internal format: probs/sevs/nivs with v/l/d and de/ha/c/t */
    var probs = [];
...
/** Analiza UNA fila con Gemini — versión autocontenida para Web App
...
function analizarFilaConIAv2(filaJSON) {
  ...
  if (!apiKey) return JSON.stringify({ error: "Sin API Key configurada. Ve al menu IA SSOMA > Configurar API Key." });
...
// Sanitizar: limpiar markdown y saltos de línea dentro de strings JSON
var limpio = content
  .replace(/```json/gi, "")
  .replace(/```/g, "")
  .trim();

// Eliminar saltos de línea reales dentro de valores string del JSON
